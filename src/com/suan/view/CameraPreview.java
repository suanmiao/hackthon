...
public class CameraPreview extends SurfaceView implements
	...
	private PreviewBitmapCallBack previewBitmapCallBack;
	...
	public CameraPreview(ViewGroup.LayoutParams layoutParams,Context context,Camera camera, LayoutMode mode) {
	...
	private void doSurfaceChanged(int width, int height) {
		if (mCamera != null) {
			...
			// The code in this if-statement is prevented from executed again
			// when surfaceChanged is
...
//				Log.e("size", width + "|" + height);
				...
				if (DEBUGGING) {
					Log.v(LOG_TAG, "Desired Preview Size - w: " + width
							+ ", h: " + height);
				}
				...
				// recursively.
				// Recursive call of surfaceChanged is very special case, which
				// is a path from
				// the catch clause at the end of this method.
				// The later part of this method should be executed as well in
				// the recursive
				// invocation of this method, because the layout change made in
				// this recursive
			...
			try {
				...
				mCamera.setPreviewCallback(new PreviewBitmapCallBack());
			} catch (Exception e) {
				...
				if (mPreviewSizeList.size() > 0) { // prevent infinite loop
					...
					Toast.makeText(getContext(), "Can't start preview",
